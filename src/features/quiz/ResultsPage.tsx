...
export const ResultsPage = () => {
  ...
  useEffect(() => {
    if (hasSaved && wasFirstOnboarding.current && user) {
      ...
      countdownRef.current = setInterval(() => { remaining -= 1; if (remaining <= 0) { clearInterval(countdownRef.current!); countdownRef.current = null; navigate(ROUTES.scanner); } else { setCountdown(remaining); } }, 1000);
    }
    ...
    const saveToProfile = async () => {
      ...
      try {
        await retryWithBackoff(async () => {
          const { error } = await supabase.from('profiles').update({ coffee_tribe: result.tribe, is_onboarded: true, onboarded_at: new Date().toISOString() }).eq('id', user.id);
          if (error) throw new Error(error.message);
        }, { maxRetries: 3, initialDelay: 1000, backoffFactor: 2 });
        setHasSaved(true);
        await refreshProfile();
        try { localStorage.removeItem(RESULT_STORAGE_KEY); localStorage.removeItem('caldi_quiz_state'); localStorage.removeItem(PENDING_TRIBE_SAVE_KEY); } catch { /* ignore */ }
    ...
    <PageLayout>
      <div className="min-h-[calc(100vh-160px)] py-12 px-4">
        <div className="max-w-2xl mx-auto">
          <div className="mb-12"><TribeReveal tribe={result.tribe} /></div>

          <div className="bg-card border-4 border-border rounded-lg p-6 shadow-[4px_4px_0px_0px_hsl(var(--border))] mb-8">
            <h2 className="font-bangers text-2xl text-foreground mb-4 tracking-wide">{t('quiz.curatedTitle')}</h2>
            <ul className="space-y-3">
              {tribeData.coffeeRecommendations.map((rec, i) => (
                <li key={i} className="flex items-center gap-3"><Coffee className="w-5 h-5 text-primary flex-shrink-0" /><span className="text-foreground">{rec}</span></li>
              ))}
            </ul>
          </div>

          <div className="bg-muted border-4 border-border rounded-lg p-6 shadow-[4px_4px_0px_0px_hsl(var(--border))] mb-8">
            <h3 className="font-bangers text-xl text-foreground mb-3 tracking-wide">{t('quiz.keywordsTitle')}</h3>
            <div className="flex flex-wrap gap-2">
              {tribeData.keywords.map((keyword) => (<span key={keyword} className="px-3 py-1 bg-background border-2 border-border rounded-full text-sm text-foreground">{keyword}</span>))}
            </div>
          </div>
          ...
          {countdown !== null && (
            <div className="bg-secondary/20 border-4 border-secondary rounded-lg p-4 mb-6 text-center">
              <div className="flex items-center justify-center gap-2 text-foreground">
                ...
                <span className="font-bangers text-lg tracking-wide">{t('quiz.redirecting')} {countdown}...</span>
              </div>
            </div>
          )}
          ...
          <div className="space-y-4">
            {user ? (
              <>
                <Button size="lg" className="w-full text-lg" onClick={() => { cancelRedirect(); navigate(ROUTES.scanner); }}><ScanLine className="w-5 h-5 mr-2" />{t('quiz.scanFirst')}</Button>
                <Button variant="outline" size="lg" className="w-full text-lg" onClick={() => { cancelRedirect(); navigate(ROUTES.dashboard); }}><LayoutDashboard className="w-5 h-5 mr-2" />{t('quiz.goToDashboard')}</Button>
              </>
            ) : (
              <Button size="lg" className="w-full text-lg" asChild><Link to={ROUTES.auth}><UserPlus className="w-5 h-5 mr-2" />{t('quiz.signUpSave')}</Link></Button>
            )}
            <Button variant="ghost" className="w-full text-muted-foreground" onClick={handleRetake}><RefreshCw className="w-4 h-4 mr-2" />{t('quiz.retakeQuiz')}</Button>
          </div>

          {user && hasSaved && (<p className="text-center text-sm text-muted-foreground mt-6">{t('quiz.tribeSaved')}</p>)}
          {user && isSaving && (<p className="text-center text-sm text-muted-foreground mt-6 animate-pulse">{t('quiz.savingTribe')}</p>)}
        </div>
      </div>
    </PageLayout>
  );
