...
export function CoffeeProfilePage() {
  ...
  if (error || !coffee) {
    return (<PageLayout><Container className="py-6"><div className="flex flex-col items-center justify-center min-h-[50vh] gap-4 text-center"><h1 className="font-bangers text-3xl text-foreground">{t('coffee.notFound')}</h1><p className="text-muted-foreground">{t('coffee.notFoundDesc')}</p><Button onClick={() => navigate(-1)} variant="outline"><ArrowLeft className="h-4 w-4 mr-2" />{t('coffee.goBack')}</Button></div></Container></PageLayout>);
  }

  return (
    <PageLayout>
      <Container className="py-6">
        <Button variant="ghost" onClick={() => navigate(scanMeta ? "/scanner" : -1 as any)} className="mb-4">
